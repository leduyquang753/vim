...
syn region javaDefaultLabelRegion matchgroup=javaLabel start="\<default\>" matchgroup=NONE end=")"
syn match   javaUserLabel	"^\s*[_$a-zA-Z][_$a-zA-Z0-9_]*\s*->"he=e-2 contains=javaLabel
...
syn match   javaNumber		 "\<\(0[bB][0-1]\+\|0[0-7]*\|0[xX]\x\+\|\d\([\s_]*\d\)*\)[lL]\=\>"
syn match   javaNumber		 "\(\<\d\([\s_]*\d\)*,\(\d\([\s_]*\d\)*\)\=\|,\d\([\s_]*\d\)*\)\([eE][-+]\=\d\([\s_]*\d\)*\)\=[fFdD]\="
syn match   javaNumber		 "\<\d\([\s_]*\d\)*[eE][-+]\=\d\([\s_]*\d\)*[fFdD]\=\>"
syn match   javaNumber		 "\<\d\([\s_]*\d\)*\([eE][-+]\=\d\([\s_]*\d\)*\)\=[fFdD]\>"
